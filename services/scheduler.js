        ...
        try {
          ...
          await notifier.notifyTenant({ pool, features: flags || {} }, {
            ...
            text: [
              ...
              `   2) ส่งรูปสลิปมาในแชท LINE นี้ได้เลย`,
              `   3) ถ้าชำระเรียบร้อยแล้ว ระบบจะอัปเดตให้อัตโนมัติ`,
              '',
              `หมายเหตุ: หากค้างชำระต่อเนื่องหลายวัน อาจมีค่าปรับเพิ่ม และบัตรผ่านประตูอาจถูกระงับชั่วคราว`,
              '',
      ...
      if (summary.changed >= 3) {
        try {
          await notifier.notifyOwner({ pool, features: _flags || {} }, {
            ...
            text: `รอบตรวจประจำวัน: ระบบปรับสถานะห้องให้ตรงกับข้อมูลผู้เช่า/สัญญาจริง ${summary.changed} ห้อง (จากที่ตรวจ ${summary.scanned} ห้อง):\n\n`
              + summary.changes.slice(0, 20).map((c) =>
                  `  • ห้อง ${c.roomId}: ${c.before || '(ว่างเปล่า)'} → ${c.after}`
                ).join('\n')
              + (summary.changes.length > 20 ? `\n  …และอีก ${summary.changes.length - 20} ห้อง` : '')
              + `\n\nปกติจะมีไม่กี่ห้องต่อวัน — ถ้าเยอะผิดสังเกต (เช่น 10+ ห้องทั้งที่ไม่มีใครย้าย) ให้เปิดเมนู "สถานะระบบ" (/admin#health) ตรวจหัวข้อความถูกต้องของข้อมูล`,
          });
        } catch { /* ignore */ }
      }
    }
    ...
    if (flatFellBack.length > 0) {
      ...
      if (!state[alertKey]) {
        ...
        notifier.notifyOwner({ pool, features: flags }, {
          category: 'billing',
          subject: `⚠️ ${flatFellBack.length} ห้องตั้งค่าเหมาน้ำ/ไฟไม่ครบ — รอบนี้คิดตามมิเตอร์แทน`,
          text: [
            `รอบบิล ${period} — ${flatFellBack.length} ห้องตั้งเป็น "เหมาจ่ายรายเดือน" ไว้ แต่ยังไม่ได้ใส่ยอดเหมา ระบบจึงคิดเงินตามมิเตอร์ให้แทนรอบนี้`,
            ``,
            ...lines,
            ``,
            `👉 วิธีแก้: เปิดหน้าห้องพัก (/admin#rooms) → เลือกห้องที่ระบุข้างต้น → ใส่ยอดเหมาค่าน้ำ/ค่าไฟให้ครบ ก่อนออกบิลรอบหน้า`,
            `(ถ้าตั้งใจให้คิดตามมิเตอร์อยู่แล้ว ให้เปลี่ยนวิธีคิดของห้องกลับเป็น "ตามมิเตอร์" จะได้ไม่แจ้งเตือนซ้ำ)`,
          ].join('\n'),
        }).catch(() => {});
        state[alertKey] = true;
        writeState(state);
    ...
    if (tenantlessSkipped.length > 0) {
      ...
      if (!state[alertKey]) {
        ...
        notifier.notifyOwner({ pool, features: flags }, {
          ...
          text: [
            `รอบบิล ${period} — ${tenantlessSkipped.length} ห้องมีชื่อผู้เช่าติดอยู่ที่ห้อง แต่ไม่พบผู้เช่าสถานะ "ใช้งาน" ในทะเบียนผู้เช่า`,
            `ระบบจึงยังไม่ออกบิลให้ห้องเหล่านี้ — ถ้าฝืนออก บิลจะไม่มีผู้รับ: ผู้เช่าจะไม่เห็นบิล ไม่ได้รับแจ้งเตือน และค่าปรับจะเพิ่มขึ้นโดยไม่มีใครรู้`,
            ``,
            ...lines,
        ...
        for (const b of billsCreated) {
          ...
          try {
            ...
            const body = [
              ...
              ``,
              `ดูรายละเอียดและวิธีชำระ — พิมพ์คำว่า "บิล" ในแชทนี้ได้เลย`,
            ].join('\n');
      ...
      try {
        ...
        for (const t of affected.rows) {
          ...
          const subject = isRevoked
            ? '🔒 บัตรผ่านประตูถูกระงับชั่วคราว — มีบิลค้างชำระ'
            : '🔓 บัตรผ่านประตูกลับมาใช้ได้แล้ว';
          const body = isRevoked
            ? [
              `เรียน คุณ${t.full_name}`,
              '',
              `บัตรผ่านประตูของคุณถูกระงับชั่วคราว เพราะมีบิลค้างชำระเกิน ${threshold} วัน`,
              billsBlock,
              `📋 วิธีแก้ (ทำได้ทันที):`,
              `   1) โอนเงินตามยอดในบิล แล้วส่งรูปสลิปมาในแชท LINE นี้ได้เลย`,
              `   2) ทันทีที่สลิปผ่านการตรวจสอบ บัตรจะกลับมาใช้ได้เอง ไม่ต้องแจ้งใคร`,
              '',
              `หากชำระแล้วแต่บัตรยังใช้ไม่ได้ ติดต่อสำนักงาน`,
            ].filter(Boolean).join('\n')
            : [
              `เรียน คุณ${t.full_name}`,
              '',
              `🎉 บัตรผ่านประตูของคุณกลับมาใช้ได้แล้ว`,
              `ขอบคุณที่ชำระบิลค้าง — ตอนนี้ไม่มีบิลค้างเกินกำหนดแล้ว`,
            ].join('\n');
    ...
    if (threshold > 1) {
      try {
        ...
        for (const t of warnQ.rows) {
          ...
          notifier.notifyTenant({ pool, features: flags || {} }, t, {
            subject: '⚠️ บัตรผ่านประตูจะถูกระงับพรุ่งนี้ — มีบิลค้างชำระ',
            text: [
              `เรียน คุณ${t.full_name}`,
              '',
              `คุณมีบิลค้างชำระใกล้ครบ ${threshold} วัน`,
              'หากยังไม่ชำระภายในวันนี้ บัตรผ่านประตูของคุณจะถูกระงับชั่วคราวในวันพรุ่งนี้',
              '',
              '📋 ทางแก้ (ทำได้ทันที):',
              '   1) โอนเงินตามยอดในบิล แล้วส่งรูปสลิปมาในแชท LINE นี้ได้เลย',
              '   2) เมื่อสลิปผ่านการตรวจสอบ บัตรจะไม่ถูกระงับ',
              '',
  ...
  try {
    ...
    for (const b of due) {
      ...
      const body = [
        ...
        `LINE ที่ผูกกับห้องนี้: ${lineBindingCount} บัญชี`,
        ``,
        `📋 วิธีชำระ: สแกน QR หรือโอนเงินตามที่ระบุในบิล แล้วส่งรูปสลิปมาในแชท LINE นี้ได้เลย`,
      ].filter(Boolean).join('\n');
